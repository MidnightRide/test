...
b8 vulkan_material_shader_create(vulkan_context *context, vulkan_material_shader *out_shader)
{
    ...
    for (u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; ++i)
    {
        ...
        {
            MERROR("Unable to create %s shader module for '%s'.", stage_type_strs[i], BUILTIN_SHADER_NAME_OBJECT);
            return false;
        }
    }
    ...
    mzero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT);
...
#define ATTRIBUTE_COUNT 2
    ...
    mzero_memory(stage_create_infos, sizeof(stage_create_infos));
    for (u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; ++i)
    {
    ...
    if (!vulkan_graphics_pipeline_create(
                                         ...
                                         &out_shader->pipeline))
    {
        MERROR("Failed to load graphics pipeline for object shader.");
        return false;
    }
    ...
    {
        MERROR("Vulkan buffer creation failed for object shader.");
        return false;
    }
    ...
    {
        MERROR("Material instance buffer creation failed for shader.");
        return false;
    }
...
void vulkan_material_shader_update_object(vulkan_context *context, struct 
                                          ...
                                          geometry_render_data data)
{
    ...
    // TODO(satvik): if needs update
    VkWriteDescriptorSet descriptor_writes[VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT];
    mzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT);
    ...
    /*
        // TODO(satvik): get diffuse colour from a material.
        static f32 accumulator = 0.0f;
        accumulator += context->frame_delta_time;
        f32 s = (msin(accumulator) + 1.0f) / 2.0f; // scale from -1, 1 to 0, 1
        obo.diffuse_color = vec4_create(s, s, s, 1.0f);
        */
    
    obo.diffuse_colour = data.material->diffuse_colour;
    ...
    for (u32 sampler_index = 0; sampler_index < sampler_count; ++sampler_index)
    {
        ...
        switch(use)
        {
            ...
            default:
            {
                MFATAL("Unable to bind sampler to unknown use.");
            } return;
        }
        ...
        if (t->generation == INVALID_ID)
        {
            ...
            // Reset the generation if using the default texture
...
b8 vulkan_material_shader_acquire_resources(vulkan_context *context, struct vulkan_material_shader *shader, material *material)
{
    ...
    {
        MERROR("Error allocating descriptor sets in shader!");
        return false;
    }
                                              ...
                                              material *material)
{
    ...
    if (result != VK_SUCCESS)
    {
        MERROR("Error freeing object shader descriptor sets!");
    }
    ...
    // TODO(satvik): add the object_id to the free list
}