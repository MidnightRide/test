...
b8 application_create(game *game_inst)
{
    ...
    game_inst->application_state = mallocate(sizeof(application_state), MEMORY_TAG_APPLICATION);
    ...
    // Logging.
    logging_system_initialise(&app_state->logging_system_memory_requirement, 0);
    ...
    if (!logging_system_initialise(&app_state->logging_system_memory_requirement, app_state->logging_system_state))
    {
        MERROR("Failed to initialise logging system; shutting down.");
        return false;
    }
    ...
    // Platform.
    platfrom_system_startup(&app_state->platform_system_memory_requirement, 0, 0, 0, 0, 0, 0);
    ...
    if (!platfrom_system_startup(&app_state->platform_system_memory_requirement, app_state->platform_system_state, game_inst->app_config.name,
    ...
    {
        MFATAL("Failed to initialise renderer. Aborting application.");
        return false;
    }
    ...
    texture_system_initialise(&app_state->texture_system_memory_requirement, 0, texture_sys_config);
    app_state->texture_system_state = linear_allocator_allocate(&app_state->systems_allocator, 
                                                                app_state->texture_system_memory_requirement);
    if (!texture_system_initialise(
                                   &app_state->texture_system_memory_requirement, 
                                   app_state->texture_system_state, 
                                   texture_sys_config))
    {
        MFATAL("Failed to initialise texture system. Aborting application.");
        return false;
    }
    ...
    material_system_initialise(&app_state->material_system_memory_requirement, 0, material_sys_config);
    app_state->material_system_state = linear_allocator_allocate(&app_state->systems_allocator, 
                                                                 app_state->material_system_memory_requirement);
    if (!material_system_initialise(
                                    &app_state->material_system_memory_requirement, 
                                    app_state->material_system_state, 
                                    material_sys_config))
    {
        MFATAL("Failed to initialise material system. Aborting application.");
        return false;
    }
    ...
    {
        MFATAL("Game failed to initialise.");
        return false;
    }
...
b8 application_run()
{
    ...
    while (app_state->is_running)
    {
        ...
        if (!app_state->is_suspended)
        {
            ...
            // TODO(satvik): refactor packet creation.
...
b8 application_on_event(u16 code, void *sender, void *listener_inst, event_context context)
{
    switch (code)
    {
        ...
        {
            MINFO("EVENT_CODE_APPLICATION_QUIT received, shutting down.\n");
            app_state->is_running = false;
            return true;
        }
    }
...
b8 application_on_key(u16 code, void *sender, void *listener_inst, event_context context)
{
    if (code == EVENT_CODE_KEY_PRESSED)
    {
        ...
        else if (key_code == KEY_A)
        {
            ...
            MDEBUG("Explicit = A key pressed!");
        }
        else
        {
            MDEBUG("'%c' key pressed in window.", key_code);
        }
    }
    else if (code == EVENT_CODE_KEY_RELEASED)
    {
        ...
        {
            // Example on checking for a key
            MDEBUG("Explicit - B key released!");
        }
        else
        {
            MDEBUG("'%c' key released in window.", key_code)
        }
    }
    return false;
}
...
b8 application_on_resized(u16 code, void *sender, void *listener_inst, event_context context)
{
    if (code == EVENT_CODE_RESIZED)
    {
        ...
        if (width != app_state->width || height != app_state->height)
        {
            ...
            MDEBUG("Window resize: %i, %i", width, height);
            
            // Handle minimization
            if (width == 0 || height == 0)
            {
                MINFO("Window minimized, suspending application.");
            ...
            else
            {
                if (app_state->is_suspended)
                {
                    MINFO("Window restored, resuming application.");
                    app_state->is_suspended = false;
                }
