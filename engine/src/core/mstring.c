...
char *string_duplicate(const char *str)
{
    ...
    char *copy = mallocate(length + 1, MEMORY_TAG_STRING);
    mcopy_memory(copy, str, length + 1);
    return copy;
}
...
i32 string_format_v(char *dest, const char *format, void *va_listp)
{
    if (dest)
    {
        ...
        buffer[written] = 0;
        mcopy_memory(dest, buffer, written + 1);
...
void string_mid(char *dest, const char *source, i32 start, i32 length)
{
    ...
    {
        // If a negative value is passed, proceed till the end of the string.
        u64 j = 0;
...
b8 string_to_vec4(char *str, vec4 *out_vector)
{
    ...
    mzero_memory(out_vector, sizeof(vec4));
...
b8 string_to_vec3(char *str, vec3 *out_vector)
{
    ...
    mzero_memory(out_vector, sizeof(vec3));
...
b8 string_to_vec2(char *str, vec2 *out_vector)
{
    ...
    mzero_memory(out_vector, sizeof(vec2));
...
b8 string_to_u8(char *str, u8 *u)
{
    ...
    mzero_memory(u, sizeof(u8));
    u32 result = sscanf(str, "%hhu", u);
    return result != -1;
}
...
b8 string_to_u16(char *str, u16 *u)
{
    ...
    mzero_memory(u, sizeof(u16));
    u32 result = sscanf(str, "%hu", u);
    return result != -1;
}
...
b8 string_to_u32(char *str, u32 *u)
{
    ...
    mzero_memory(u, sizeof(u32));
    u32 result = sscanf(str, "%u", u);
    return result != -1;
}
...
b8 string_to_u64(char *str, u64 *u)
{
    ...
    mzero_memory(u, sizeof(u64));
    u32 result = sscanf(str, "%llu", u);
    return result != -1;
}
