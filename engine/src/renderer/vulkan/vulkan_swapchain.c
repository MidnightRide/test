    ...
    u32 *out_image_index)
{
    ...
    {
        MFATAL("Failed to acquire swapchain image!");
        return false;
    }
    ...
    u32 present_image_index)
{
    ...
    {
        MFATAL("Failed to present swap chain image!");
    }
...
void create(vulkan_context *context, u32 width, u32 height, vulkan_swapchain *swapchain)
{
    ...
    swapchain_extent.width = MCLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = MCLAMP(swapchain_extent.height, min.height, max.height);
    ...
    if (!swapchain->images)
    {
        swapchain->images = (VkImage *)mallocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views)
    {
        swapchain->views = (VkImageView *)mallocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    ...
    if (!vulkan_device_detect_depth_format(&context->device))
    {
        ...
        MFATAL("Failed to find a supported format!");
    }
    ...
    MINFO("Swapchain created successfully.");
}
