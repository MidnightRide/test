                           ...
                           i32 width, i32 height)
{
    *memory_requirement = sizeof(platform_state);
    ...
    wc.lpszClassName = "matcha_window_class";
    ...
    HWND handle = CreateWindowExA(
                                  window_ex_style, "matcha_window_class", application_name,
    ...
    // Show the window
    b32 should_activate = 1; // TODO(satvik): if the window should not accept input, this should be false.
    i32 show_window_command_flags = should_activate ? SW_SHOW : SW_SHOWNOACTIVATE;
    // If initially minimized, use SW_MINIMIZE : SW_SHOWNOACTIVATE;
    // If initially maximised, use SW_MAXIMIZE : SW_SHOWNOACTIVATE;
...
b8 platform_create_vulkan_surface(struct vulkan_context *context)
{
    ...
    VK_CHECK(vkCreateWin32SurfaceKHR(context->instance, &create_info, context->allocator, &state_ptr->surface));
...
LRESULT CALLBACK win32_process_message(HWND hwnd, u32 msg, WPARAM w_param, LPARAM l_param)
{
    switch (msg)
    {
        ...
        case WM_MOUSEMOVE:
        {
            ...
            // Pass over to the input subsystem for processing.
