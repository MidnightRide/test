...
b8 platfrom_system_startup(platform_state *plat_state, const char *application_name, i32 x, i32 y, i32 width, i32 height)
{
    ...
    xcb_change_property(
                        ...
                        XCB_ATOM_WM_NAME,
                        XCB_ATOM_STRING,
                        8, // data should be veiwed 8 bits at a time
    ...
    xcb_intern_atom_cookie_t wm_protocols_cookie = xcb_intern_atom(
                                                                   ...
                                                                   "WM_PROTOCOLS");
    xcb_intern_atom_cookie_t *wm_delete_reply = xcb_intern_atom_reply(
                                                                      state->connection,
                                                                      wm_delete_cookie,
                                                                      NULL);
    xcb_intern_atom_cookie_t *wm_protocols_reply = xcb_intern_atom_reply(
                                                                         state->connection,
                                                                         wm_protocols_cookie,
                                                                         NULL);
    ...
    xcb_change_property(
                        ...
                        4,
                        32, // data should be veiwed 8 bits at a time
                        1,
    ...
    if (stream_result <= 0)
    {
        MFATAL("An error occurred when flushing the stream: %d", stream_result);
        return false;
    }
...
void platform_system_shutdown(platform_state *plat_state)
{
    ...
    // Turn on key repeats since this is global for the OS
...
b8 platform_pump_messages(platform_state *plat_state)
{
    ...
    // Poll for the events until null is returned
    whlie(event != 0)
    {
...
b8 platform_create_vulkan_surface(struct platform_state *plat_state, struct vulkan_context *context)
{
    ...
    VK_CHECK(vkCreateXcbSurfaceKHR(context->instance, &create_info, context->allocator, &state->surface));
...
keys translate_keycode(u32 x_keycode)
{
    switch (x_keycode)
    {
        ...
        case XK_Meta_L:
        return KEY_LWIN; // TODO(satvik): not sure this is right
        case XK_Meta_R:
        return KEY_RWIN;
